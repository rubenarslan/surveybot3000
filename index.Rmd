...
bind_rows(
  ...
  accuracy_bayes %>% 
    mutate(model = "fine-tuned", kind = "semi-latent (Bayesian EIV)") %>% 
    select(model, kind, accuracy = r, conf.low = r.lower, conf.high = r.upper)
  ) %>% 
  knitr::kable(digits = 2)
```
...
plot_rels + ylab("") + xlab("Synthetic Estimates (Θ)") + ggtitle(with(accuracy_bayes_rels, { sprintf("Reliabilities r=%.2f [%.2f;%.2f]", r, r.lower, r.upper) })) +
  theme(plot.title = element_text(size = 11)) +
plot_scales + ylab("") + xlab("") + ggtitle(with(accuracy_bayes_scales, { sprintf("Scale pairs r=%.2f [%.2f;%.2f]", semi_latent_r, .lower, .upper) })) +
  theme(plot.title = element_text(size = 11))
...
(plot_items  + ggtitle(with(accuracy_bayes_items, { sprintf("accuracy = %.2f [%.2f;%.2f]", semi_latent_r, .lower, .upper) })) +
    theme(plot.title = element_text(size = 11)) +
    plot_rels + ggtitle(with(accuracy_bayes_rels, { sprintf("accuracy = %.2f [%.2f;%.2f]", semi_latent_r, .lower, .upper) })) +
    theme(plot.title = element_text(size = 11)) +
    plot_scales + ggtitle(with(accuracy_bayes_scales, { sprintf("accuracy = %.2f [%.2f;%.2f]", semi_latent_r, .lower, .upper) })) +
    theme(plot.title = element_text(size = 11)))
