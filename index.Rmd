...
### Prediction error according to predicted value
```{r}
...
find_reverse_items_by_first_item <- function(rs, first_item_key) {
  ...
  items <- rs[-1, 1]
  if(first_item_key == 1) {
    reversed <- which(items < 0)
  } else {
    reversed <- which(items > 0)
  }
  reverse_keyed_items <- names(items)[reversed]
  reverse_keyed_items
}
...
### Prediction error according to predicted value and no. of items
```{r}
(m_lmsynth_r_scales <- brm(
  bf(empirical_alpha | mi(empirical_alpha_se) ~ synthetic_alpha,
     sigma ~ synthetic_alpha + number_of_items), data = scales, 
