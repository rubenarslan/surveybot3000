...
accuracy_bayes <- m_synth_r_items %>% 
  tidy_draws() %>% 
  mutate(
    sigma = exp(b_sigma_Intercept),
    total_var = (sd_synth*b_synthetic_r) ^ 2 + sigma^2 + sd_mmvariable_1variable_2__Intercept^2,
    exp_var = (sd_synth*b_synthetic_r) ^ 2 / total_var,
    r = sqrt(exp_var)) %>% 
  mean_hdci(sigma, exp_var, r)

accuracy_bayes_items <- accuracy_bayes
...
bind_rows(
  ...
  accuracy_bayes %>% 
    ...
    select(kind, accuracy = r, conf.low = r.lower, conf.high = r.upper)
  ) %>% 
...
sd_synth <- sd(m_lmsynth_rel_scales$data$synthetic_alpha)

accuracy_bayes <- m_lmsynth_rel_scales %>% 
  tidy_draws() %>% 
  mutate(
    sigma = exp(b_sigma_Intercept), # poly terms have zero means and drop
                                    # out of the equation
    total_var = (sd_synth*b_synthetic_alpha) ^ 2 + sigma^2,
    exp_var = (sd_synth*b_synthetic_alpha) ^ 2 / total_var,
    r = sqrt(exp_var)) %>% 
  mean_hdci(sigma, exp_var, r)
accuracy_bayes_rels <- accuracy_bayes
...
accuracy_bayes <- m_lmsynth_r_scales %>% 
  tidy_draws() %>% 
  mutate(
    sigma = exp(b_sigma_Intercept),
    total_var = (sd_synth*b_synthetic_r) ^ 2 + sigma^2 + sd_mmscale_ascale_b__Intercept^2,
    exp_var = (sd_synth*b_synthetic_r) ^ 2 / total_var,
    r = sqrt(exp_var)) %>% 
  mean_hdci(sigma, exp_var, r)
accuracy_bayes_scales <- accuracy_bayes
...
plot_items + ylab("Empirical Estimates (r/α)") + xlab("") + ggtitle(with(accuracy_bayes_items, { sprintf("Item pairs r=%.2f [%.2f;%.2f]", r, r.lower, r.upper) })) +
  theme(plot.title = element_text(size = 11)) +
plot_rels + ylab("") + xlab("Synthetic Estimates (Θ)") + ggtitle(with(accuracy_bayes_rels, { sprintf("Reliabilities r=%.2f [%.2f;%.2f]", r, r.lower, r.upper) })) +
  theme(plot.title = element_text(size = 11)) +
plot_scales + ylab("") + xlab("") + ggtitle(with(accuracy_bayes_scales, { sprintf("Scale pairs r=%.2f [%.2f;%.2f]", r, r.lower, r.upper) })) +
  theme(plot.title = element_text(size = 11))

plot_items  + ggtitle(with(accuracy_bayes_items, { sprintf("r=%.2f [%.2f;%.2f]", r, r.lower, r.upper) })) +
    theme(plot.title = element_text(size = 11)) +
    plot_rels + ggtitle(with(accuracy_bayes_rels, { sprintf("r=%.2f [%.2f;%.2f]", r, r.lower, r.upper) })) +
    theme(plot.title = element_text(size = 11)) +
    plot_scales + ggtitle(with(accuracy_bayes_scales, { sprintf("r=%.2f [%.2f;%.2f]", r, r.lower, r.upper) })) +
    theme(plot.title = element_text(size = 11))

ggsave("ignore/Figure_pilot.pdf", width = 8.3, height = 5.5, device = grDevices::cairo_pdf)
```
