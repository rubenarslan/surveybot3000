...
Re-runnable from scratch:
    ...
    python pooling_comparison/run_pooling_comparison.py

Inputs (all already in this repo, except the validation-study embeddings, which
are downloaded from the surveybot3000 GitHub repo on first run and cached):

    data/intermediate/ItemSimilarityTraining-20240502-trial12.raw.<dataset>.{mapping2,scales,human,machine,scale_correlations}.feather

Outputs go to results/pooling_comparison/.
...
DATA = os.path.join(ROOT, "data", "intermediate")
OUT = os.path.join(ROOT, "results", "pooling_comparison")
VAL_EMB_URL = (
    "https://github.com/synth-science/surveybot3000/raw/refs/heads/main/"
    "validation_study/embeddings_surveybot3000.feather"
)
VAL_EMB_CACHE = os.path.join(OUT, "cache", "embeddings_surveybot3000.feather")
...
def main():
    ...
    md.append(
        "Generated by `pooling_comparison/run_pooling_comparison.py`. Pooling functions live in "
        "`pooling_comparison/scale_pooling.py` (numpy only)."
    )
    md.append("")
    md.append(
    ...
    md.append("- `cache/embeddings_surveybot3000.feather`: validation-study item embeddings (copied from the surveybot3000 GitHub repo)")
    md.append("")
