...
epreds <- epred_draws(newdata = newdata, obj = m_synth_r_items_poly, re_formula = NA)
preds <- predicted_draws(newdata = newdata, obj = m_synth_r_items_poly, re_formula = NA)
epred_preds <- epreds %>% left_join(preds)
by_draw <- epred_preds %>% group_by(.draw) %>% 
  summarise(.epred = var(.epred),
            .prediction = var(.prediction),
...
ggplot(aes(x = domain, r, ymin = conf.low, ymax = conf.high, shape = kind, fill = kind)) +
  ...
  scale_y_continuous("Manifest accuracy", limits = c(0, 1)) +
  geom_text(aes(label = sprintf("%.2f", r)), hjust = -0.3, vjust = -0.4, size = 3, position = position_dodge(width = dodge))
...
rr_validation_llm %>% filter(abs(empirical_r) >= .10) %>% 
  group_by(sign(empirical_r)) %>% 
  summarise(mean(sign(empirical_r) == sign(synthetic_r)))

rr_validation_llm %>% 
  summarise(mean( sign(synthetic_r) > 0), mean( sign(empirical_r) > 0))

cor_switch <- rr_validation_llm %>% mutate(synthetic_r = if_else(sign(synthetic_r) == sign(empirical_r), synthetic_r,
                                                                 -1*synthetic_r)) %>% 
 { broom::tidy(cor.test(.$synthetic_r, .$empirical_r)) }
...
m_lmsynth_rel_scales_s1 <- brm(
  bf(empirical_alpha | mi(empirical_alpha_se) ~ synthetic_alpha,
     sigma ~ s(synthetic_alpha)), data = s1_scales, 
