...
rr_validation <- arrow::read_feather(file = file.path(data_path, glue("data/intermediate/{model_name}.raw.validation-study-2024-11-01.item_correlations.feather")))

pt_rr_validation <- arrow::read_feather(file = file.path(data_path, glue("data/intermediate/{pretrained_model_name}.raw.validation-study-2024-11-01.item_correlations.feather")))
...
```{r}
accuracy_between_domains <- rr_validation_llm_domain %>% 
  filter(DomainA != DomainB),
...
```{r}
accuracy_between_instruments <- rr_validation_llm_domain %>% 
...
##### Top/bottom 20 items
```{r}
by_item <- item_pair_table_numeric,
  item_pair_table_numeric %>% 
...
```{r}
leaked <- rio::import("data/search_results.parquet") %>% 
...
items <- item_pair_table %>% select(Instrument = InstrumentA, item_textl = item_text_1l),
                   item_pair_table %>% select(Instrument = InstrumentB, item_textl = item_text_2l)) %>% 
  distinct()
...
rr_validation_llm_leakage <- item_pair_table %>% 
...
```{r}
scales <- readRDS(file = file.path(data_path, glue("data/intermediate/scales_with_alpha_se_rr.rds")))
...
```{r}
manifest_scores = arrow::read_feather(file = file.path(data_path, glue("data/intermediate/{model_name}.raw.validation-study-2024-11-01.scale_correlations.feather")))
pt_manifest_scores = arrow::read_feather(file = file.path(data_path, glue("data/intermediate/{pretrained_model_name}.raw.validation-study-2024-11-01.scale_correlations.feather")))
