...
bind_rows(
  ...
  kable(digits = 2, caption = str_c("Accuracy for k=",nrow(rr_validation_llm)," item pairs (",n_distinct(c(rr_validation_llm$variable_1, rr_validation_llm$variable_2))," items) across language models and methods"))
```
...
ggsave("Figure_rr_domain.pdf", width = 6, height = 5)
```
...
ggplot(aes(y = `exclusion criteria`, x= accuracy, xmin = conf.low, xmax = conf.high, color = kind)) +
  geom_pointrange() + 
  scale_color_viridis_d("Type of correlation", end = 0.7) +
  geom_text(aes(label = sprintf("%.2f", accuracy)), vjust = -1, show.legend = F) +
  xlab("Accuracy") +
  ylab("Exclusion criteria") +
  theme(legend.position = c(.25,0.9), legend.background = element_rect(linetype = "solid", color = 'gray', linewidth = 0.4))
ggsave("Figure_exclusion.png", height = 5, width = 7)
ggsave("Figure_exclusion.pdf", height = 5, width = 7)
```
