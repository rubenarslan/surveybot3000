...
accuracy_within_domains <- rr_validation_llm_domain %>% 
  ...
  select(domain = DomainA, r = estimate, conf.low, conf.high, pt_cor, n, sd_emp_r) %>% 
  arrange(r)
...
outside = accuracy_between_domains, .id = "kind") %>% 
ggplot(aes(x = domain, r, ymin = conf.low, ymax = conf.high, color = kind)) +
  ...
  coord_flip()
ggsave("Figure_rr_domain.png", width = 4, height = 5)
```
...
outside = accuracy_between_instruments, .id = "kind") %>% 
ggplot(aes(x = instrument, r, ymin = conf.low, ymax = conf.high, color = kind)) +
  geom_point(aes(y = pt_cor, group = kind), color = "gray", position = position_dodge(width = 0.3)) +
  geom_linerange(aes(ymin = pt_cor, ymax = r, group = kind), color = "gray", position = position_dodge(width = 0.3)) +
  geom_pointrange(position = position_dodge(width = 0.3)) + 
  scale_color_viridis_d(end = 0.7, option = "A", guide = F) +
